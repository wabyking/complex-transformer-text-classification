...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1,tgt_emb_prj_weight_sharing=False):
        ...
        self.phase =nn.Linear(d_model, d_model)
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)
        if tgt_emb_prj_weight_sharing:
            self.real.weight = self.phase.weight

        self.linears_real = clones(self.real, 4)
        self.linears_phase = clones(self.phase, 4)
        # self.dropout = nn.Dropout(p=dropout)
        # self.attn = None
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, real, phase, mask=None):
        "Implements Multi-head attention"
        if mask is not None:
            # Same mask applied to all h heads.
            mask = mask.unsqueeze(1)
        nbatches = real.size(0)
        ...
        query_real, key_real, value_real = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears_real, (real, real, real))]

        query_phase, key_phase, value_phase = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears_phase, (phase, phase, phase))]

        # 2) Apply attention on all the projected vectors in batch.
        x_real, x_phase, self.attn = attention(query_real, key_real, value_real, query_phase, key_phase, value_phase,
                                               mask=mask,
                                               dropout=self.dropout)
        ...
        return self.linears_real[-1](x_real) + real, self.linears_phase[-1](x_phase) + phase
